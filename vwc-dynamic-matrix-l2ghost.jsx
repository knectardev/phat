...
// Quadrant metadata — L2-Ghost axes.
// Names/tags are intentionally dropped: the four archetype cells (The Slip,
// Institutional Buy, Hidden Floor, Coherence) serve as the matrix's conceptual
// anchors. Quadrant-level names would collide with them — only corners are
// archetypes; the other 24 cells per quadrant are graduations toward them.
// Colors remain so the matrix background tints still render.
const QUAD_META = {
  1: { color: '#5fa8a8' },   // phat volume, thin book  (Slip region)
  2: { color: '#6ba368' },   // phat volume, phat book  (Engine region)
  3: { color: '#8a8374' },   // thin volume, thin book  (Lull region)
  4: { color: '#c75c5c' },   // thin volume, phat book  (Hidden Floor region)
};

// Vector naming — cleared for the L2 pivot. The old six transitions
// (Fake-out / Confirmation / Climax / Exhaustion / Ignition / Thinning) were
// defined against the Effort×Result axes and do not port to Volume×Depth.
// The detection effect becomes a no-op; VectorTicker renders a placeholder.
const VECTOR_NAMES = {};

// ─── Dense cell taxonomy (100 cells) ──────────────────────────────
// Ported verbatim from prototypes/fractal_phatness_matrix.html (lines 597–699).
// Keyed by `${y}_${x}` where y = Price Phatness bucket (executed volume
// percentile), x = Ghost Phatness bucket (L2 depth percentile), both 1..10.
const STATE_DATA = {
  // ROW 10: CLIMAX (Highest Price Volume)
  '10_1':  { name: 'Blowout',          desc: 'Panic',           quad: 'q1' },
  '10_2':  { name: 'Flash Crash',      desc: 'Vol Void',        quad: 'q1' },
  '10_3':  { name: 'Market Crash',     desc: 'Capitulation',    quad: 'q1' },
  '10_4':  { name: 'Slip Spike',       desc: 'Thin Bid',        quad: 'q1' },
  '10_5':  { name: 'The Slip',         desc: 'Ghost Trap',      quad: 'q1', key: true },
  '10_6':  { name: 'Conviction Gap',   desc: 'High Vol',        quad: 'q2' },
  '10_7':  { name: 'Heavy Engine',     desc: 'High Both',       quad: 'q2' },
  '10_8':  { name: 'Bull Run',         desc: 'Climax',          quad: 'q2' },
  '10_9':  { name: 'The Engine',       desc: 'Full Conviction', quad: 'q2', key: true },
  '10_10': { name: 'Institutional Buy',desc: 'Phat Climb',      quad: 'q2' },
  // ROW 9
  '9_1':   { name: 'Liquidation',      desc: 'Panic Low',       quad: 'q1' },
  '9_2':   { name: 'Gap Down',         desc: 'Vol Drop',        quad: 'q1' },
  '9_3':   { name: 'Falling Knife',    desc: 'Fast Drop',       quad: 'q1' },
  '9_4':   { name: 'Fear Spike',       desc: 'Hollow',          quad: 'q1' },
  '9_5':   { name: 'Exhaustion',       desc: 'Thin L2',         quad: 'q1' },
  '9_6':   { name: 'Momentum',         desc: 'Building',        quad: 'q2' },
  '9_7':   { name: 'Growing',          desc: 'Trend Up',        quad: 'q2' },
  '9_8':   { name: 'Breakout',         desc: 'Trending',        quad: 'q2' },
  '9_9':   { name: 'Rally',            desc: 'High Energy',     quad: 'q2' },
  '9_10':  { name: 'Surge',            desc: 'Phat Rise',       quad: 'q2' },
  // ROW 8
  '8_1':   { name: 'Shock Drop',       desc: 'Vol Panic',       quad: 'q1' },
  '8_2':   { name: 'Sell Vol',         desc: 'Dumping',         quad: 'q1' },
  '8_3':   { name: 'Unwind',           desc: 'Liquidation',     quad: 'q1' },
  '8_4':   { name: 'Flush Run',        desc: 'Thin Depth',      quad: 'q1' },
  '8_5':   { name: 'Weakness',         desc: 'Selling',         quad: 'q1' },
  '8_6':   { name: 'Strength',         desc: 'Buying Vol',      quad: 'q2' },
  '8_7':   { name: 'Trend Form',       desc: 'Direction',       quad: 'q2' },
  '8_8':   { name: 'Push',             desc: 'Directional',     quad: 'q2' },
  '8_9':   { name: 'Strong Trend',     desc: 'Conviction',      quad: 'q2' },
  '8_10':  { name: 'Institutional',    desc: 'Major Buy',       quad: 'q2' },
  // ROW 7
  '7_1':   { name: 'Sharp Drop',       desc: 'Vol Shock',       quad: 'q1' },
  '7_2':   { name: 'Heavy Sell',       desc: 'Volume',          quad: 'q1' },
  '7_3':   { name: 'Distribution',     desc: 'Selling',         quad: 'q1' },
  '7_4':   { name: 'Pressure',         desc: 'Downside',        quad: 'q1' },
  '7_5':   { name: 'Bearish Vol',      desc: 'High Pressure',   quad: 'q1' },
  '7_6':   { name: 'Bullish Vol',      desc: 'High Demand',     quad: 'q2' },
  '7_7':   { name: 'Building Trend',   desc: 'Forming',         quad: 'q2' },
  '7_8':   { name: 'Trend Up',         desc: 'Momentum',        quad: 'q2' },
  '7_9':   { name: 'Bull Strength',    desc: 'Confident',       quad: 'q2' },
  '7_10':  { name: 'Big Buy',          desc: 'Institutional',   quad: 'q2' },
  // ROW 6
  '6_1':   { name: 'Heavy Down',       desc: 'Vol Down',        quad: 'q1' },
  '6_2':   { name: 'Selling Fest',     desc: 'Bearish',         quad: 'q1' },
  '6_3':   { name: 'Heavy Sell Off',   desc: 'Distribution',    quad: 'q1' },
  '6_4':   { name: 'Down Pressure',    desc: 'Bearish',         quad: 'q1' },
  '6_5':   { name: 'Heavy Weakness',   desc: 'Vol Pressure',    quad: 'q1' },
  '6_6':   { name: 'Heavy Strength',   desc: 'Vol Demand',      quad: 'q2' },
  '6_7':   { name: 'Strong Form',      desc: 'Trend Setup',     quad: 'q2' },
  '6_8':   { name: 'Strong Trend',     desc: 'Momentum',        quad: 'q2' },
  '6_9':   { name: 'Bull Climax',      desc: 'High Conviction', quad: 'q2' },
  '6_10':  { name: 'Major Rally',      desc: 'Phat Buy',        quad: 'q2' },
  // ROW 5
  '5_1':   { name: 'Moderate Down',    desc: 'Medium Vol',      quad: 'q1' },
  '5_2':   { name: 'Selling',          desc: 'Medium Vol',      quad: 'q1' },
  '5_3':   { name: 'Mild Dist',        desc: 'Gradual',         quad: 'q1' },
  '5_4':   { name: 'Thin Gap',         desc: 'High Vol',        quad: 'q1' },
  '5_5':   { name: 'Climax',           desc: 'Mixed',           quad: 'mixed' },
  '5_6':   { name: 'Rise',             desc: 'Building',        quad: 'q2' },
  '5_7':   { name: 'Moderate Up',      desc: 'Vol Up',          quad: 'q2' },
  '5_8':   { name: 'Buying',           desc: 'Medium Vol',      quad: 'q2' },
  '5_9':   { name: 'Bull Phase',       desc: 'Growing',         quad: 'q2' },
  '5_10':  { name: 'Ascent',           desc: 'Climbing',        quad: 'q2' },
  // ROW 4
  '4_1':   { name: 'Fade Down',        desc: 'Light Vol',       quad: 'q1' },
  '4_2':   { name: 'Mild Sell',        desc: 'Light Vol',       quad: 'q1' },
  '4_3':   { name: 'Soft Down',        desc: 'Drifting',        quad: 'mixed' },
  '4_4':   { name: 'Volume Run',       desc: 'Mixed',           quad: 'mixed' },
  '4_5':   { name: 'Strong',           desc: 'Balanced',        quad: 'mixed' },
  '4_6':   { name: 'Building',         desc: 'Growing',         quad: 'mixed' },
  '4_7':   { name: 'Vol Up',           desc: 'Increasing',      quad: 'q2' },
  '4_8':   { name: 'Mild Buy',         desc: 'Light Vol',       quad: 'q2' },
  '4_9':   { name: 'Momentum',         desc: 'Growing',         quad: 'q2' },
  '4_10':  { name: 'Steady Rise',      desc: 'Climbing',        quad: 'q2' },
  // ROW 3
  '3_1':   { name: 'Drift Low',        desc: 'Hollow',          quad: 'q3' },
  '3_2':   { name: 'Wander',           desc: 'Light',           quad: 'q3' },
  '3_3':   { name: 'Coil',             desc: 'Spring Coil',     quad: 'q3' },
  '3_4':   { name: 'Choppy',           desc: 'Mixed',           quad: 'mixed' },
  '3_5':   { name: 'Coherence',        desc: 'Balanced',        quad: 'mixed', key: true },
  '3_6':   { name: 'Stable',           desc: 'Steady',          quad: 'mixed' },
  '3_7':   { name: 'Transition',       desc: 'Forming',         quad: 'mixed' },
  '3_8':   { name: 'Grind',            desc: 'Steady',          quad: 'q4' },
  '3_9':   { name: 'Sticky',           desc: 'Resistant',       quad: 'q4' },
  '3_10':  { name: 'Anchored',         desc: 'Support',         quad: 'q4' },
  // ROW 2
  '2_1':   { name: 'Thin Void',        desc: 'Weightless',      quad: 'q3' },
  '2_2':   { name: 'Apathy',           desc: 'Light',           quad: 'q3' },
  '2_3':   { name: 'Lull',             desc: 'Quiet',           quad: 'q3' },
  '2_4':   { name: 'Absorption',       desc: 'Quiet',           quad: 'q4' },
  '2_5':   { name: 'Neutral',          desc: 'Low Activity',    quad: 'mixed' },
  '2_6':   { name: 'Silent Bid',       desc: 'Buyers',          quad: 'q4' },
  '2_7':   { name: 'Accumulation',     desc: 'Quiet',           quad: 'q4' },
  '2_8':   { name: 'Building Wall',    desc: 'Orders',          quad: 'q4' },
  '2_9':   { name: 'Heavy Wall',       desc: 'Resistance',      quad: 'q4' },
  '2_10':  { name: 'Major Support',    desc: 'Anchored',        quad: 'q4' },
  // ROW 1: ANEMIC (Lowest Price Volume)
  '1_1':   { name: 'Void',             desc: 'Weightless',      quad: 'q3' },
  '1_2':   { name: 'Drift',            desc: 'Apathy',          quad: 'q3' },
  '1_3':   { name: 'Coil',             desc: 'Spring',          quad: 'q3' },
  '1_4':   { name: 'Silent',           desc: 'Orders',          quad: 'q4' },
  '1_5':   { name: 'Hidden Trap',      desc: 'Danger',          quad: 'q4' },
  '1_6':   { name: 'Silent Buyer',     desc: 'Accumulating',    quad: 'q4' },
  '1_7':   { name: 'Absorption',       desc: 'Quiet',           quad: 'q4' },
  '1_8':   { name: 'Wall Build',       desc: 'Resting',         quad: 'q4' },
  '1_9':   { name: 'Hidden Floor',     desc: 'Accumulation',    quad: 'q4', key: true },
  '1_10':  { name: 'Institutional Bid',desc: 'Major Buyers',    quad: 'q4' },
};

// ─── Archetype overlay (4 corners + 1 center) ─────────────────────
// Pedagogical anchors that don't replace the dense names above — they sit on
// top of them. Corner archetypes mark the four true extremes of the phase
// space; the center anchor marks the balance point. Every other cell shows
// only its STATE_DATA dense name. (1,1) intentionally has no archetype overlay.
const ARCHETYPES = {
  '10_1':  { name: 'The Slip',          kind: 'corner' },
  '10_10': { name: 'Institutional Buy', kind: 'corner' },
  '1_10':  { name: 'Hidden Floor',      kind: 'corner' },
  '5_5':   { name: 'Coherence',         kind: 'center' },
};

// Cell-level taxonomy lookup for hover tooltip.
//   x = Ghost Phatness bucket (L2 depth percentile), 1..10
//   y = Price Phatness bucket (volume percentile), 1..10
// Returns { headline, body, edge } for the hover card. STATE_DATA supplies
// the dense per-cell name; ARCHETYPES (4 corners + 1 center) adds an italic
// archetype subscript at the conceptual extremes of the phase space.
function cellTaxonomy(x, y /* , quad */) {
  const key = `${y}_${x}`;
  const state = STATE_DATA[key];
  const archetype = ARCHETYPES[key];
  const depthStr = x <= 3 ? 'hollow' : x <= 7 ? 'balanced' : 'phat';
  const volStr   = y <= 3 ? 'anemic' : y <= 7 ? 'active'   : 'climactic';
  const headline = state ? `${state.name} · [${x}, ${y}]` : `[${x}, ${y}]`;
  const descTail = state?.desc ? ` · ${state.desc}` : '';
  const body     = `${volStr.charAt(0).toUpperCase() + volStr.slice(1)} volume, ${depthStr} book${descTail}.`;
  const edge     = archetype ? archetype.name : '';
  return { headline, body, edge, archetype };
}
...
function generateBook(midPrice, bookBias, phaseId) {
  ...
  // L2-Ghost aggregate: total resting depth within ±0.5% of mid (or 20 ticks
  // minimum — so tiny/synthetic prices still see enough levels). Used for the
  // new Ghost-Phatness (X) percentile axis; kept separate from bidTot/askTot/obi
  // so existing callers and the close-N OBI signal stay untouched.
  const bandSize = Math.max(midPrice * 0.005, 20 * TICK);
  const bidDepth = bids
    .filter(b => b.price >= midPrice - bandSize)
    .reduce((s, b) => s + b.size, 0);
  const askDepth = asks
    .filter(a => a.price <= midPrice + bandSize)
    .reduce((s, a) => s + a.size, 0);
  const totalDepth = bidDepth + askDepth;
  const bookImbalance = (bidDepth - askDepth) / (totalDepth || 1);
  ...
  return {
    ...
    spread,
    mid: midPrice,
    l2: { bidDepth, askDepth, totalDepth, bookImbalance },
  };
}
  ...
  useEffect(() => {
    ...
    tickRef.current = setInterval(() => {
      setPhaseTick(pt => {
        ...
        // Attach synthetic Level 2 book BEFORE ranking — the new Ghost-Phatness
        // (X) axis needs `book.l2.totalDepth` to exist at bucketing time.
        ...
        setCandles(prev => {
          ...
          // ── L2-Ghost coordinate ranking ──
          // Y (Price Phatness) = percentile rank of executed volume across the window.
          // X (Ghost Phatness) = percentile rank of total L2 depth (±0.5% of mid band)
          // across the window. `bodyPct` is still computed because downstream doji
          // detection below keys off body-range percentile, independent of the axes.
          const vols = next.map(c => c.vol);
          const depths = next.map(c => c.book?.l2?.totalDepth ?? 0);
          const bodies = next.map(c => c.earnedBody);
          const volPct = percentileRank(vols, newCandle.vol);
          const depthPct = percentileRank(depths, newCandle.book.l2.totalDepth);
          const bodyPct = percentileRank(bodies, newCandle.earnedBody);
          const x = pctToBucket(depthPct);
          const y = pctToBucket(volPct);
          newCandle.volPct = volPct;
          newCandle.depthPct = depthPct;
          ...
          // Note: `newCandle.book` was attached before this callback so that the
          // new Ghost-Phatness (X) axis could rank by `book.l2.totalDepth`.
...
// Dual-layer glyph for the L2-Ghost phase space.
//   • Ghost rect  = resting book infrastructure; width = f(x) only.
//   • Price rect  = executed volume body; width = f(y) only.
//   • Height is CONSTANT across all cells — this is a phase-space reference
//     diagram, not a time series. Each cell renders the archetypal candle
//     for its coordinate; no random `heightRandomness` or sin() demo tricks.
//
// Color decoupling (per notes.txt):
//   priceColor from V/D = y/x        (execution intensity vs. infrastructure)
//   ghostColor from x alone           (resting-book presence)
// Divergence Glow fires when priceColor !== ghostColor — a visible "Phake-Out"
// where price action disagrees with the book context.
function cellColors(x, y) {
  const vd = x > 0 ? y / x : 1;
  const priceColor =
    vd < 0.8 ? '#21a095' :          // teal   — depth exceeds volume (absorption)
    vd > 1.2 ? '#fb7185' :          // red    — volume exceeds depth (distribution)
               '#7a8195';           // gray   — balanced
  const ghostColor =
    x >= 7   ? '#00d9ff' :          // cyan   — phat resting depth
    x <= 3   ? '#fb7185' :          // red    — hollow book
               '#a0a9b8';           // gray   — ambiguous
  return { priceColor, ghostColor, divergent: priceColor !== ghostColor };
}

function CellGlyph({ x, y, opacity = 1 }) {
  const VB = 100;
  const cx = VB / 2;
  const cy = VB / 2;

  // Geometry — deterministic, one axis per dimension, height constant.
  const ghostWidth   = 15 + ((x - 1) / 9) * 70;   // 15..85 at x=1..10
  const priceWidth   = 15 + ((y - 1) / 9) * 70;   // 15..85 at y=1..10
  const candleHeight = 60;                         // constant for every cell
  const halfH        = candleHeight / 2;

  const { priceColor, ghostColor, divergent } = cellColors(x, y);

  // CSS drop-shadow is cheaper than an SVG filter and is applied to the whole
  // SVG so the glow sits outside the candle silhouette. Two stacked shadows
  // (amber + cyan) give Phake-Outs an unmistakable aura.
  const glowFilter = divergent
    ? 'drop-shadow(0 0 4px rgba(212,168,75,0.55)) drop-shadow(0 0 8px rgba(0,217,255,0.30))'
    : 'none';
  ...
  return (
    ...
    >
      {/* GHOST — resting depth. Drawn first, translucent. */}
      <line
        x1={cx} y1={cy - halfH - 8}
        x2={cx} y2={cy + halfH + 8}
        stroke={ghostColor} strokeWidth="0.8" opacity="0.35"
      />
      <rect
        x={cx - ghostWidth / 2} y={cy - halfH}
        width={ghostWidth} height={candleHeight}
        fill={ghostColor} fillOpacity={0.22}
        stroke={ghostColor} strokeWidth="0.6" strokeOpacity="0.6"
      />

      {/* PRICE — executed volume body. Drawn second, opaque. */}
      <line
        x1={cx} y1={cy - halfH - 6}
        x2={cx} y2={cy + halfH + 6}
        stroke={priceColor} strokeWidth="1" opacity="0.9"
      />
      <rect
        x={cx - priceWidth / 2} y={cy - halfH}
        width={priceWidth} height={candleHeight}
        fill={priceColor} fillOpacity={0.9}
...
function Matrix({ bias, activeCoord, trailCoords, activeQuad, pulseId, reversal, obi, smooth, onHover, onLeave }) {
  ...
  for (let r = 0; r < GRID; r++) {
    ...
    for (let c = 0; c < GRID; c++) {
      ...
      // Archetype overlay shadow — corners get amber-gold halo, center gets a
      // subtle teal ring. Applied only when the cell is NOT the active/trail
      // cell (active/trail highlight takes priority to keep the live path
      // legible).
      const archetype = ARCHETYPES[`${y}_${x}`];
      let archetypeShadow;
      if (archetype && !isActive && trailIdx < 0) {
        archetypeShadow = archetype.kind === 'corner'
          ? 'inset 0 0 0 1.5px rgba(212,168,75,0.75), 0 0 10px rgba(212,168,75,0.35)'
          : 'inset 0 0 0 1px rgba(95,168,168,0.55)';
      }
          ...
          style={{
            ...
            boxShadow: isActive
              ...
              : trailIdx >= 0
                ...
                : archetypeShadow,
            zIndex: isActive ? 5 : trailIdx >= 0 ? 3 : archetype ? 2 : 1,
          }}
          onMouseEnter={() => onHover(x, y, q)}
          onMouseLeave={onLeave}
        >
          <CellGlyph x={x} y={y}
                     opacity={isActive ? 1 : trailIdx === 0 ? 0.85 : trailIdx === 1 ? 0.55 : 0.55}/>
          {isActive && (
    ...
    <Tip block className="space-y-2" text="Recent named quadrant transitions detected on the regime-aware quad path (see pattern key for definitions).">
      ...
      <div className="space-y-1 min-h-[140px] max-h-[140px] overflow-hidden">
        {vectorHistory.length === 0 && (
          <div className="text-[10px] text-[var(--fg-dim)] italic tracking-wide font-['Fraunces']">
            Vector taxonomy pending re-derivation for L2 axes.
          </div>
        )}
...
function TrendStrengthPanel({ trend, profile, regimeFilter }) {
  const { score, rawScore, direction, signals, tier, reason, pending } = trend;
  const accent = '#22d3ee';
  ...
  if (pending) {
    return (
      <div
        className="bg-[var(--bg-alt)] border p-5 relative overflow-hidden flex flex-col min-h-[472px]"
        style={{ borderColor: `rgba(${accentRgb},0.35)` }}
      >
        <div className="flex items-center gap-2 mb-3">
          <TrendingUp size={12} style={{ color: '#5b6b78' }} strokeWidth={1.5}/>
          <span className="text-[9px] tracking-[0.25em] uppercase text-[var(--fg-dim)]">Continuation Detector</span>
        </div>
        <div className="flex-1 flex items-center justify-center text-center px-6">
          <div>
            <div className="font-['Fraunces'] italic text-base text-[var(--fg)] mb-2">
              Law set pending re-derivation for L2 axes.
            </div>
            <div className="text-[11px] text-[var(--fg-dim)] leading-relaxed">
              The five continuation laws were defined against Effort × Result. Volume × L2-Depth
              needs its own laws (e.g. Depth Collapse, Ghost Alignment) before this panel can
              emit a conviction signal.
            </div>
          </div>
        </div>
      </div>
...
function ReversalSignal({ reversal, currentQuad, profile, regimeFilter, mtfConfluenceActive = false }) {
  const { score, rawScore, significance, inertiaBonus, direction, signals, target, pending } = reversal;
  ...
  if (pending) {
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          <Zap size={12} style={{ color: '#5b6b78' }} strokeWidth={1.5}/>
        ...
        <div className="flex-1 flex items-center justify-center text-center px-6">
          <div>
            ...
            <div className="text-[11px] text-[var(--fg-dim)] leading-relaxed">
              The five reversal laws (Fat Reversal, Vacuum Snap, Curvature, Divergence, Liquidity
              Divergence) were tuned for Effort × Result. Volume × L2-Depth needs its own laws
              (e.g. Ghost Divergence, Depth Collapse) before this panel can emit a confidence score.
            </div>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  const reversal = useMemo(() => {
    // ── L2-Ghost pivot ──
    // The five reversal laws below (Fat Reversal, Vacuum Snap, Curvature,
    // Divergence, Liquidity Divergence) were defined against the Effort×Result
    // axes. On the new Volume × L2-Depth axes they do not translate by a simple
    // relabel, so the detector is stubbed with `pending: true`. ReversalSignal
    // renders a placeholder card when it sees this flag. The original law code
    // is left in place below as dead reference material for the re-derivation.
    return { score: 0, direction: null, primaryLaw: null, signals: [], target: null, pending: true };
  ...
  const trendStrength = useMemo(() => {
    // ── L2-Ghost pivot ──
    // The five continuation laws below were defined against Effort×Result. They
    // are stubbed with `pending: true` pending re-derivation for Volume × L2-Depth.
    // TrendStrengthPanel renders a placeholder card when it sees this flag.
    return { score: 0, rawScore: 0, direction: null, tier: 'NONE', signals: [],
             primaryLaw: null, reason: 'pending re-derivation for L2 axes', pending: true };
    // eslint-disable-next-line no-unreachable
    const neutral = (over = {}) => ({
      score: 0,
      rawScore: 0,
  ...
  const depthPctDisplay = current && current.depthPct != null ? (current.depthPct * 100).toFixed(0) : '—';
      ...
      <div className="relative" style={{ padding: isMobile ? '12px 0 20px' : '20px 0 32px' }}>
        ...
        <header className="flex items-end justify-between border-b border-[var(--line-strong)] pb-5 mb-5 gap-4 flex-wrap px-5">
          <div className="flex items-baseline gap-4 flex-wrap">
            ...
            <Tip block text="Demo harness for Volume (Y) × L2-Depth (X) coordinates on a 10×10 phase-space matrix, with dual-layer ghost candles and a scripted cycle. Not connected to live markets.">
              <div>
                <h1 className="font-['Fraunces'] text-[22px] leading-tight tracking-tight">L2-Ghost Matrix</h1>
                <div className="text-[10px] tracking-[0.25em] uppercase text-[var(--fg-dim)] mt-0.5">
                  Volume × L2-Depth Phase Space · Scripted Cycle
                </div>
              </div>
            </Tip>
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4 px-5 py-3 bg-[var(--bg-alt)] border border-[var(--line)] mb-5">
          ...
          <div className="flex gap-5 text-[10px] uppercase tracking-[0.15em]">
            ...
            <Tip text="Structural quadrant of the displayed coordinate on the Volume (Y) × L2 Depth (X) matrix.">
              <div>
                <span className="text-[var(--fg-mute)]">Quad</span> <span className="text-[var(--fg)] font-medium">{activeQuad ? `Q${activeQuad}` : '—'}</span>
              </div>
            </Tip>
        ...
        <div style={{
          ...
          <aside className="space-y-4" style={{ order: isDesktop ? 0 : 2 }}>
            ...
            <div className="bg-[var(--bg-alt)] border border-[var(--line)] p-5 space-y-3">
              ...
              <StatRow title="Price Phatness (Y): where the current bar’s executed volume sits in the rolling lookback window." label="Vol Percentile" value={current ? `${volPctDisplay}` : '—'} suffix="th"/>
              <StatRow title="Ghost Phatness (X): where the current bar’s L2 total depth (±0.5% of mid band) sits in the rolling lookback window." label="Depth Percentile" value={current ? `${depthPctDisplay}` : '—'} suffix="th"/>
              <StatRow title="Where the bar’s body size sits in the rolling lookback window (secondary signal, used for doji detection)." label="Body Percentile" value={current ? `${bodyPctDisplay}` : '—'} suffix="th"/>
              <StatRow title="Displayed matrix coordinate [x,y] = [Ghost Phatness, Price Phatness]: kinetic-smoothed when the regime filter is on." label="Coordinate" value={activeCoord ? `[${activeCoord.x}, ${activeCoord.y}]` : '—'} accent/>
              <StatRow title="Structural quadrant of the active cell. Quadrants are regions; the four corners carry archetype names (The Slip / Institutional Buy / Hidden Floor / Coherence at center)." label="Quadrant"
                       value={activeQuad ? `Q${activeQuad}` : '—'}
                       subvalue=""/>
            ...
            {/* LEGEND — ARCHETYPE CORNERS */}
            <div className="bg-[var(--bg-alt)] border border-[var(--line)] p-5">
              <div className="flex items-center gap-2 mb-3">
                <CircleDot size={12} className="text-[var(--amber)]" strokeWidth={1.5}/>
                <Tip text="Four corner archetypes plus the center anchor. The dense 100-cell taxonomy populates every cell; the archetypes mark the conceptual extremes of the Volume × L2-Depth phase space.">
                  <span className="text-[9px] tracking-[0.25em] uppercase text-[var(--fg-dim)]">Archetype Key</span>
                </Tip>
              </div>
              <div className="space-y-2.5 text-[11px]">
                {[
                  { coord: '[10, 1]',  name: 'The Slip',          tag: 'Phat price through a hollow book.'     , color: '#d4a84b' },
                  { coord: '[10, 10]', name: 'Institutional Buy', tag: 'Execution matches infrastructure.'     , color: '#d4a84b' },
                  { coord: '[1, 10]',  name: 'Hidden Floor',      tag: 'Anemic price, resting wall of depth.'  , color: '#d4a84b' },
                  { coord: '[5, 5]',   name: 'Coherence',         tag: 'Center anchor — balanced on both axes.', color: '#5fa8a8' },
                ].map(a => (
                  <div key={a.coord} className="flex items-start gap-2.5">
                    <div className="w-2 h-2 mt-1.5 shrink-0 rounded-full" style={{ background: a.color, boxShadow: `0 0 6px ${a.color}` }}/>
                    <div>
                      <div><span className="font-['Fraunces'] italic text-[var(--fg)]">{a.name}</span>
                        <span className="text-[var(--fg-mute)] text-[9px] tracking-widest ml-1.5">{a.coord}</span>
                      </div>
                      <div className="text-[10px] text-[var(--fg-dim)] leading-snug">{a.tag}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Pattern / vector key — stubbed for L2 pivot */}
            <div className="bg-[var(--bg-alt)] border border-[var(--line)] p-4">
              <Tip block text="The old Fake-out / Confirmation / Climax / Exhaustion / Ignition / Thinning taxonomy was defined against the Effort×Result axes. It does not port to Volume × L2-Depth and needs to be re-derived.">
                <div className="flex items-center gap-2 mb-3">
              ...
              <div className="text-[11px] text-[var(--fg-dim)] leading-relaxed font-['Fraunces'] italic">
                Vector taxonomy pending re-derivation for L2 axes.
              </div>
            </div>
          </aside>
            ...
            <div className="bg-[var(--bg-alt)] border border-[var(--line)] p-5 relative">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Tip text="10×10 phase-space: Price Phatness (Y / executed volume percentile) × Ghost Phatness (X / L2 depth percentile). Each cell renders the archetypal dual-layer candle for its coordinate; highlighted cell is the live coordinate; dashed trail shows prior path.">
                      <span className="text-[9px] tracking-[0.25em] uppercase text-[var(--fg-dim)]">10×10 L2-Ghost Matrix</span>
                    </Tip>
                    {regimeFilter && (
              ...
              <div className="relative pl-10 pb-10">
                {/* Y axis label — Price Phatness (executed volume) */}
                <div className="absolute left-0 top-1/2 -translate-y-1/2 -rotate-90 origin-center text-[9px] tracking-[0.28em] uppercase text-[var(--fg-dim)] whitespace-nowrap">
                  Price Phatness · Executed Volume  (1 anemic → 10 climax) →
                </div>
                {/* X axis label — Ghost Phatness (L2 depth) */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-[9px] tracking-[0.28em] uppercase text-[var(--fg-dim)]">
                  Ghost Phatness · L2 Depth  (1 hollow → 10 phat) →
                </div>
                ...
                <div className="relative">
                  ...
                  {/* Archetype corner labels (overlay, non-blocking).
                      Placed at the true extreme corners of the phase space:
                        top-left     = (x=1,  y=10)  Hidden Floor
                        top-right    = (x=10, y=10)  Institutional Buy
                        bottom-left  = (x=1,  y=1)   — dense taxonomy only, no archetype
                        bottom-right = (x=10, y=1)   The Slip                       */}
                  <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute top-2 left-2">
                      <div className="text-[8px] tracking-[0.25em] uppercase text-[var(--fg-mute)]">[1, 10]</div>
                      <div className="font-['Fraunces'] italic text-xs" style={{ color: '#d4a84b', textShadow: '0 0 6px rgba(212,168,75,0.45)' }}>Hidden Floor</div>
                    </div>
                    <div className="absolute top-2 right-2 text-right">
                      <div className="text-[8px] tracking-[0.25em] uppercase text-[var(--fg-mute)]">[10, 10]</div>
                      <div className="font-['Fraunces'] italic text-xs" style={{ color: '#d4a84b', textShadow: '0 0 6px rgba(212,168,75,0.45)' }}>Institutional Buy</div>
                    </div>
                    <div className="absolute bottom-2 right-2 text-right">
                      <div className="text-[8px] tracking-[0.25em] uppercase text-[var(--fg-mute)]">[10, 1]</div>
                      <div className="font-['Fraunces'] italic text-xs" style={{ color: '#d4a84b', textShadow: '0 0 6px rgba(212,168,75,0.45)' }}>The Slip</div>
                    </div>
                    {/* Center archetype — Coherence at (5,5). Positioned slightly
                        above center so it doesn't clash with the matrix gridline. */}
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-[calc(50%+18px)] text-center">
                      <div className="text-[8px] tracking-[0.25em] uppercase text-[var(--fg-mute)]">[5, 5]</div>
                      <div className="font-['Fraunces'] italic text-xs" style={{ color: '#5fa8a8', textShadow: '0 0 4px rgba(95,168,168,0.35)' }}>Coherence</div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 pt-4 border-t border-[var(--line)] min-h-[70px]">
                {hoverInfo ? (
                  <div>
                    ...
                    {hoverInfo.edge && (
                      <div
                        className="text-[12px] italic mt-2 font-['Fraunces']"
                        style={{
                          color: hoverInfo.archetype?.kind === 'corner' ? '#d4a84b' : '#5fa8a8',
                        }}
                      >
                        {hoverInfo.archetype?.kind === 'corner' ? '★ ' : '◉ '}{hoverInfo.edge}
                        <span className="text-[9px] tracking-[0.2em] uppercase text-[var(--fg-mute)] ml-2 not-italic font-sans">
                          {hoverInfo.archetype?.kind === 'corner' ? 'archetype · corner' : 'archetype · center'}
                        </span>
                      </div>
                    )}
                  </div>
          ...
          <aside className="space-y-4" style={{ order: isDesktop ? 0 : 3 }}>
            ...
            {/* JOINT READOUT */}
            <Tip text="Joint readout maps continuation strength tier × reversal tier to a narrative phrase (presentation only; no additional scoring). Stubbed while the L2-axis law sets are pending re-derivation." block>
              <div className="bg-[var(--bg-alt)] border border-[var(--line)] px-4 py-2.5">
                ...
                <div className="text-[11px] text-[var(--fg-dim)] font-['Fraunces'] italic leading-snug">
                  {(smoothedTrendStrength.pending || smoothedReversal.pending)
                    ? 'Joint readout pending re-derivation for L2 axes.'
